...
import omc.boundbyfate.client.kool.TextureCopyHelper
...
object MCGlApi: GlApi {
    ...
    lateinit var backend: RenderBackendGl
    ...
    override fun lineWidth(width: Float) = Unit // Р’ СЃРѕРІСЂРµРјРµРЅРЅРѕРј OpenGL С‚Р°РєРѕРµ РЅРµР»СЊР·СЏ
    ...
    override fun texImage1d(data: ImageData1d) {
        texImage2dImpl(GL_TEXTURE_2D, data)
    }
    override fun texImage2d(target: Int, level: Int, internalformat: Int, width: Int, height: Int, border: Int, format: Int, type: Int, pixels: Buffer?) {
        texImage2dImpl(target, level, internalformat, width, height, border, format, type, pixels)
    }
    ...
    fun initOpenGl(backend: MCRenderBackendGl) {
        ...
        val extensions = (0..<extCount).map { glGetStringi(GL_EXTENSIONS, it) }
        ...
        capabilities = GlCapabilities(
            ...
            hasClipControl = false, // Р”Р»СЏ СЃРѕРІРјРµСЃС‚РёРјРѕСЃС‚Рё СЃ СЂРµРЅРґРµСЂРёРЅРіРѕРј РёРіСЂС‹ (РѕРЅР° РёСЃРїРѕР»СЊР·СѓРµС‚ GL_LOWER_LEFT Рё GL_NEGATIVE_ONE_TO_ONE)
