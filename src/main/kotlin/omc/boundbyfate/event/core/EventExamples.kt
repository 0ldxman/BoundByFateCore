...
import omc.boundbyfate.event.content.ACCalculationEvent
import omc.boundbyfate.event.content.CharacterCreateEvent
import omc.boundbyfate.event.content.CharacterEvents
import omc.boundbyfate.event.content.CombatEvents
import omc.boundbyfate.event.content.DamageEvent
import omc.boundbyfate.event.content.StatChanged
...
object EventExamples {
    ...
    fun example1_SimpleSubscription() {
        CharacterEvents.STAT_CHANGED.register { entity, statId, oldValue, newValue ->
    ...
    fun example2_PrioritySubscription() {
        CharacterEvents.STAT_CHANGED.register(EventPriority.HIGH) { entity, statId, oldValue, newValue ->
            println("HIGH: Validating stat change...")
        }

        CharacterEvents.STAT_CHANGED.register(EventPriority.NORMAL) { entity, statId, oldValue, newValue ->
            println("NORMAL: Processing stat change...")
        }

        CharacterEvents.STAT_CHANGED.register(EventPriority.LOW) { entity, statId, oldValue, newValue ->
    ...
    fun example3_CancelEvent() {
        CharacterEvents.BEFORE_CREATE.register(EventPriority.HIGH) { event ->
        ...
        CharacterEvents.BEFORE_CREATE.invoke { it.onBeforeCreate(event) }
    ...
    fun example4_ModifyResult() {
        CombatEvents.BEFORE_DAMAGE.register(EventPriority.HIGH) { event ->
            event.result = event.result * 1.5f
            println("Damage increased: ${event.originalResult} -> ${event.result}")
        }

        CombatEvents.BEFORE_DAMAGE.register(EventPriority.NORMAL) { event ->
        ...
        CombatEvents.BEFORE_DAMAGE.invoke { it.onBeforeDamage(event) }
        println("Final damage: ${event.result}")
    }
    ...
    fun example5_CancelWithMonitor() {
        ...
        CombatEvents.BEFORE_DAMAGE.register(EventPriority.MONITOR) { event ->
            println("MONITOR: Logging damage attempt (cancelled: ${event.isCancelled})")
        }

        val event = DamageEvent(
            target = mockEntity(),
            source = SourceReference.ability(Identifier("boundbyfate-core", "magic_missile")),
            amount = 10f
        )

        CombatEvents.BEFORE_DAMAGE.invokeCancellable(event) { it.onBeforeDamage(event) }
    }
    ...
    fun example6_EventChain() {
        CharacterEvents.STAT_CHANGED.register { entity, statId, oldValue, newValue ->
            if (statId.toString() == "boundbyfate-core:dexterity") {
                ...
                CombatEvents.CALCULATE_AC.invoke { it.onCalculateAC(acEvent) }
            }
        }

        CombatEvents.CALCULATE_AC.register { event ->
            val dex = 14
    ...
    fun example7_DynamicUnsubscribe() {
        ...
        CharacterEvents.STAT_CHANGED.register(handler = handler)
        CharacterEvents.STAT_CHANGED.unregister(handler)
    }
    ...
    class ExampleSystem {
        fun initialize() {
            CharacterEvents.LEVEL_UP.register(EventPriority.NORMAL) { entity, oldLevel, newLevel ->
